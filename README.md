...
The latest updates can be found in the [CHANGELOG](./CHANGELOG.md).
...
`RBlind.css` goes to a custom directory `/rblind/`.
...
Note if using the `sassc` library the argument will be `sassc` instead. Note that importing css files (like `RBlind.css`) must exclude the extension `.css`. This method does not work with ruby based sass libraries as noted in this [Stack Overflow thread](https://stackoverflow.com/questions/7111610/import-regular-css-file-in-scss-file).
...
4. `@import "./rblind/RBlind";` our custom code
...
If you are loading the themes with an external style sheet and notice it has not applied correctly, try adding `@import "./rblind/RBlind` before the bootstrap import so it is loaded twice. When loaded as a local theme in Settings the additional import is not required.
...
All css classes defined are in `RBlind.css`.
